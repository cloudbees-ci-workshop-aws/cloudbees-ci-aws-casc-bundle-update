          ...
          stages {
              ...
              steps {
                container("kubectl") {
                  ...
                  sh '''
                    ...
                    kubectl cp --namespace cloudbees-sda ${BUNDLE_ID} cjoc-0:/var/jenkins_config/jcasc-bundles-store/ -c jenkins
                  '''
            ...
            stage('Auto Reload Bundle') {
              ...
              steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'admin-cli-token', usernameVariable: 'JENKINS_CLI_USR', passwordVariable: 'JENKINS_CLI_PSW')]) {
                  sh '''
                    curl --user $JENKINS_CLI_USR:$JENKINS_CLI_PSW -XGET -H "Accept: application/json" http://${BUNDLE_ID}.cloudbees-sda.svc.cluster.local/${BUNDLE_ID}/casc-bundle-mgnt/check-bundle-update
                    curl --user $JENKINS_CLI_USR:$JENKINS_CLI_PSW -XPOST -H "Accept: application/json" http://${BUNDLE_ID}.cloudbees-sda.svc.cluster.local/${BUNDLE_ID}/casc-bundle-mgnt/reload-bundle
                  '''
                }
              }
            }
